...
import pygame.mixer as mixer        # install pygame
import os
...
# Finalizing the GUI
root.update()
# .load() - Loading the song into playlist from listbox
#  play() - Play the song
# set()  - Setting the status
# pause() - Pausing the music
# unpause() - Unpausing the song are in the pygame
# StringVar, ListBox are used to declare variables in the tkinter window.
...
def load(listbox):
    os.chdir(filedialog.askdirectory(title='Select your directory'))
...
button_frame = LabelFrame(root, text='Controls', bg='Turquoise', width=400, height=120)
...
# SongFrame Labels
Label(song_frame, text='Playing', bg='LightBlue', font=('Times', 10, 'bold')).place(x=5, y=20)

song_lbl = Label(song_frame, textvariable=current_song, bg='Goldenrod', font=("Times", 12), width=25)
song_lbl.place(x=150, y=20)

# Setting buttons and intializing the buttons
